...
## 2026-09-24 — Live browser clock

The browser now supplies its local date/time and time zone at connection and when the minute or zone changes, with catch-up after returning to the tab. Updates enter the live conversation silently through `session.thinking.append`; ordinary time questions no longer delegate to Hermes. Startup server time remains a fallback until the first browser update. Answers use minute precision and readable zone names. The worker's separate timestamp behavior is unchanged.

Clock updates reuse acknowledged context delivery, discard superseded pending clock versions, and stop/reset on disconnect. The static module is served and included in the build identifier. No Hermes or Hindsight source or configuration changes are required.

All 44 live regression tests passed, including midnight, daylight-saving changes, time-zone changes, missed/rejected updates, paused-tab catch-up, and reconnect. Real browser audio checks confirmed correct answers before and after a minute rollover and a live change from Pacific to Japan time, with zero Hermes delegations and no unsolicited clock announcement. Received speech began in 0.97–1.34 seconds. Content was checked against the provider transcript with received audio captured; this is not a physical phone/speaker test. Detailed evidence is kept in private local state.

Reference: [Microsoft GPT-Live context updates](https://learn.microsoft.com/azure/foundry/openai/how-to/gpt-live#add-context-during-the-conversation).
