...
## Current app replaced at the user's request

The user requested replacing the old app with the researched version after the remaining acceptance failures were explained. After the user's two live background jobs completed and conversation paused, `voice-frontend.service` was switched to build `cef1bf62525899d7` at 2026-09-21 23:49 UTC. Its normal URL now serves the new build. The preview service is stopped, and port 8443 aliases the same current service. Personal history/task/memory state was backed up and preserved; original settings/source remain for rollback. Health, original routes and state counts were verified. See `deploy/live-production/README.md`. The user also reported positive experience with the live preview; performance/phone acceptance has not been falsely marked complete.
