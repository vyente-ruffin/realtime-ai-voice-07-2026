...
## Agreed-build checks after the shutdown fix

Build `768d6764e3d31a33` was checked with the existing spoken cases. The older-memory request and background calculation both completed through real Hermes jobs; the voice returned the Museum of Glass in Tacoma, the correct total of 2,870, and Jasmine tea. Both job outcomes were offered once. The background case exceeded its 20-second completed-response window, so this remains a failed timing check. The follow-up landed after the calculation finished; this run does not add evidence for conversation during running work. The earlier successful concurrent-work check remains the evidence for that behavior. Initial sound includes acknowledgements and other asynchronous results and is not counted as the requested meaningful answer. Evidence: `agreed-build-work/`.

The existing interruption test now waits for the full recorded question to finish and for sustained assistant speech before interrupting. All five inputs were separate and all five produced the requested “Ready” response, without continuing the story before the next request. None met the 500ms stop target: one measured gap began after 1,363ms; four had no 250ms quiet gap in the measurement window. Continuous transition to a new reply can obscure a silence-based measure, but the output transcripts also show the old story continuing beyond 500ms in every attempt. Evidence: `agreed-build-interruptions/`.

Both runners now exit unsuccessfully when their recorded acceptance checks fail. No application behavior, architecture, provider settings, or dependency source was changed during these checks. The isolated service was stopped. Existing latency failures, the unexplained long delay, and phone acceptance remain open; these results do not authorize production deployment.

## Documented prompt structure

The current build `cef1bf62525899d7` now retains the named interruption policy and separate delegation/do-not-delegate sections required by the [official GPT-Live prompting guide](https://developers.openai.com/api/docs/guides/live-prompting) and present in the installed Hermes reference. The earlier prompt omitted those labels. No transport, model, deployment, memory settings or upstream code changed.

The same three spoken work cases returned the museum answer from available context and completed one real calculation job with the correct result spoken once. The tea follow-up had no input or output transcript. The generic runner counted audio from the preceding task as a response, so its successful exit is not accepted as a passing conversation check. The museum question did not invoke retrieval on this run and is not an uncached-recall measurement. All five interruption cases produced the requested Ready response, but none met the existing 500ms quiet-gap check. Evidence and these limits are in `documented-policy-work/summary.json` and `documented-policy-interruptions/summary.json`. This brings the implementation into line with the cited instructions; it does not establish a latency fix or deployment acceptance. The isolated service is stopped.
